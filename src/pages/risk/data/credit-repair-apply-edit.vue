...
const onDeleteFile = e => {
  if (e.file.id) {
    const params = { 'creditRepairApplyDatum.id': file.id }
    request('/creditRepairApplyDeleteUploadAttach.action', params)
  }
...
const onSubmit = () => {
  refForm.value
    .validate()
    .then(() => {
      creditRepairApplyEditSave()
    })
    .catch(() => uni.$u.toast('校验失败'))
}

onLoad(options => {
  id = options['creditRepairApply.Id']
  creditRepairApplyEdit()
})

const creditRepairApplyEdit = () => {
  const params = { 'creditRepairApply.Id': id }
  request('/creditRepairApplyEdit.action', params).then(res => {
    Object.assign(creditRepairApply, reactive(res))
  })
}
/* 提交业务逻辑 */
const creditRepairApplyEditSave = () => {
  const params = {
  ...
  request('/creditRepairApplyEditSave.action', params)
    .then(res => {
      upload(id, formData.value.filePath)
    })
